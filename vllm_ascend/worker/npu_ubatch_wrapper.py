...
from vllm_ascend.worker.ubatching import AscendUBatchContext, make_ubatch_contexts
...
class AscendUBatchWrapper(UBatchWrapper):
    ...
    def clear_graphs(self) -> None:
        ...
        if self.cudagraph_wrapper is not None:
            # UBatchWrapper.clear_graphs assumes CUDA's CUDAGraphWrapper has a
            # clear_graphs method. Ascend ACLGraphWrapper stores entries here.
            self.cudagraph_wrapper.concrete_aclgraph_entries.clear()

    def __getattr__(self, key: str):
        if hasattr(self.runnable, key):
            return getattr(self.runnable, key)
        raise AttributeError(f"Attribute {key} not found in AscendUBatchWrapper runnable.")
    ...
    def _run_ubatches(self, ubatch_metadata: list[AscendUbatchMetadata], model) -> torch.Tensor | IntermediateTensors:
    ...
    def _capture_ubatches(self, ubatch_metadata: list[AscendUbatchMetadata], model) -> torch.Tensor:
        results: list[tuple[int, Any]] = []
        ...
        with override_forward_context(None):
            ...
            with torch.npu.graph(cudagraph_metadata.aclgraph, stream=compute_stream, pool=self.graph_pool):
                ...
                for thread in ubatch_threads:
                    thread.join()
                sorted_results = [value for _, value in sorted(results)]
                cudagraph_metadata.outputs = self._merge_outputs(sorted_results, ubatch_metadata)
    ...
    def _slice_model_inputs(self, tokens_slice: slice, input_ids, positions, inputs_embeds, intermediate_tensors):
        ...
        sliced_positions = positions[:, tokens_slice] if positions.ndim == 2 else positions[tokens_slice]
    ...
    def _merge_intermediate_tensors(self, intermediate_tensor_list):
        ...
        for key in intermediate_tensor_list[0].tensors:
            result[key] = torch.cat(
                [intermediate_tensor_list[0].tensors[key], intermediate_tensor_list[1].tensors[key]],
                dim=0,
            )
        return IntermediateTensors(result)

    def _merge_outputs(self, sorted_results: list[Any], ubatch_metadata: list[AscendUbatchMetadata]):
    ...
    def __call__(self, *args, **kwargs):
        ...
        logger.debug(
            "AscendUBatchWrapper running %s ubatches: slices=%s, cudagraph_runtime_mode=%s, num_tokens=%s",
            len(ubatch_slices),
            ubatch_slices,
            cudagraph_runtime_mode,
            num_tokens,
        ...
        if num_tokens in self.cudagraphs and cudagraph_runtime_mode is CUDAGraphMode.FULL:
